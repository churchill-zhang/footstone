...
std::mutex WorkerManager::creation_mutex_;

std::shared_ptr<WorkerManager> WorkerManager::instance_ = nullptr;
...
std::shared_ptr<WorkerManager> WorkerManager::GetDefaultManager() {
  std::scoped_lock creation(creation_mutex_);
  if (!instance_) {
    instance_ = std::make_shared<WorkerManager>(kDefaultSize);
  }
  return instance_;
}
...
void WorkerManager::MoveTaskRunnerSpecificNoLock(int32_t runner_id,
...
void WorkerManager::Balance(int32_t increase_worker_count) {
  if (increase_worker_count > 0) {
    ...
    TDF_BASE_DCHECK(!reduce_size) << "resize failed, The thread has too many unschedulable runners";
    if (reduce_size > 0) {
      return;
    }
    for (int i = 0; i < size; ++i) {
